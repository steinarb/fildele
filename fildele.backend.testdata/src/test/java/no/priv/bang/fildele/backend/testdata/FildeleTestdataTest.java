...
class FildeleTestdataTest {

    @Test
    void testActivate() {
        var testdata = new FildeleTestdata();
        ...
        when(useradmin.getRoles()).thenReturn(Arrays.asList(Role.with().build(), Role.with().build(), Role.with().id(2).rolename(FILDELEUSER_ROLE).build()));
